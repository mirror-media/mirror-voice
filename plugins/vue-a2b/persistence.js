...
export const storage = {
  ...
  isUniversal: false, // use cookie-universal for setting and getting cookie if true, avoid SSR issue
  expiry: 30, // ignore entries that weren't touched this amount of days

  _load() {
    if (this.method === 'cookie') {
      this._store = getCookie(this.name, this.isUniversal) || {}
  ...
  _save() {
    if (this.method === 'cookie')
      writeCookie(this.name, this._store, this.expiry, this.isUniversal)
    else if (this.method === 'localStorage')
